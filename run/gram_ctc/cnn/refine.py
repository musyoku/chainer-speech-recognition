...
def main():
	...
	loader = Loader(
		...
		buckets_cache_size=0, 						# ディスクから読み込んだデータをメモリにキャッシュする個数
		...
		try:
			with chainer.using_config("train", False):
				with chainer.no_backprop_mode():
					...
					printr("Calculating usage of grams ... {}/{}".format(batch_index + 1, total_iterations))
					y_batch = model(x_batch, split_into_variables=False)
					y_batch = xp.argmax(y_batch.data, axis=2)
					y_batch = xp.ravel(y_batch)
					nonzero = xp.nonzero(y_batch)	# blankは無視
